...
/-!
# Source-derived formalization layer for `invariant-subspace-problem-canonical-lane`
...
def sourceFormulaModels : List SourceFormulaModel := [
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", expr := (FormulaExpr.var "eps_coh_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/INVARIANT_SUBSPACE_PROBLEM_PREPRINT.md Appendix E.6", notes := "Strict coherence target.", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"], components := [
    { key := "eps_coh_raw", value := "0.0" }
  ] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", expr := (FormulaExpr.div (FormulaExpr.num "1.0") (FormulaExpr.add (FormulaExpr.num "1.0") (FormulaExpr.var "delta_comp_sup_raw"))), parseStatus := "parsed_source_expression", sourceSection := "paper/INVARIANT_SUBSPACE_PROBLEM_PREPRINT.md Section 5.3 / Appendix C", notes := "kappa_compact theorem-level floor.", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"], components := [
    { key := "delta_comp_sup_raw", value := "0.244" }
  ] },
  { group := "constants", key := "kappa_operator", status := "derived_numeric", formula := "c_operator_raw * operator_density_raw - e_operator_raw", expr := (FormulaExpr.sub (FormulaExpr.mul (FormulaExpr.var "c_operator_raw") (FormulaExpr.var "operator_density_raw")) (FormulaExpr.var "e_operator_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/INVARIANT_SUBSPACE_PROBLEM_PREPRINT.md Section 4.5 / Appendix A", notes := "kappa_operator theorem-level floor.", validation := "required_positive", componentKeys := ["c_operator_raw", "e_operator_raw", "operator_density_raw"], components := [
    { key := "c_operator_raw", value := "1.45" },
    { key := "e_operator_raw", value := "0.235" },
    { key := "operator_density_raw", value := "0.916" }
  ] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", expr := (FormulaExpr.var "rho_rigidity_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/INVARIANT_SUBSPACE_PROBLEM_PREPRINT.md Section 6.1 / Appendix D", notes := "rho_rigidity theorem-level floor.", validation := "required_positive", componentKeys := ["rho_rigidity_raw"], components := [
    { key := "rho_rigidity_raw", value := "1.077" }
  ] },
  { group := "constants", key := "sigma_spectral", status := "derived_numeric", formula := "spectral_floor_raw - perturbation_loss_raw - restart_loss_raw", expr := (FormulaExpr.sub (FormulaExpr.sub (FormulaExpr.var "spectral_floor_raw") (FormulaExpr.var "perturbation_loss_raw")) (FormulaExpr.var "restart_loss_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/INVARIANT_SUBSPACE_PROBLEM_PREPRINT.md Section 5.2 / Appendix B", notes := "sigma_spectral theorem-level floor.", validation := "required_positive", componentKeys := ["perturbation_loss_raw", "restart_loss_raw", "spectral_floor_raw"], components := [
    { key := "perturbation_loss_raw", value := "0.168" },
    { key := "restart_loss_raw", value := "0.148" },
    { key := "spectral_floor_raw", value := "1.391" }
  ] },
  { group := "constants", key := "subspace_transfer", status := "derived_numeric", formula := "c_subspace_raw * transfer_gain_raw - e_subspace_raw", expr := (FormulaExpr.sub (FormulaExpr.mul (FormulaExpr.var "c_subspace_raw") (FormulaExpr.var "transfer_gain_raw")) (FormulaExpr.var "e_subspace_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/INVARIANT_SUBSPACE_PROBLEM_PREPRINT.md Section 6.2 / Appendix E.4", notes := "subspace_transfer theorem-level floor.", validation := "required_positive", componentKeys := ["c_subspace_raw", "e_subspace_raw", "transfer_gain_raw"], components := [
    { key := "c_subspace_raw", value := "1.118" },
    { key := "e_subspace_raw", value := "0.121" },
    { key := "transfer_gain_raw", value := "1.029" }
  ] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", expr := (FormulaExpr.var "sigma_star_can_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/INVARIANT_SUBSPACE_PROBLEM_PREPRINT.md Section 5 / stitch constants", notes := "sigma_star_can theorem-level stitch constant.", validation := "required_positive", componentKeys := ["sigma_star_can_raw"], components := [
    { key := "sigma_star_can_raw", value := "1.053" }
  ] }
]

def formalizationCertificate : FormalizationCertificate := {
  sourceRepo := "invariant-subspace-problem-canonical-lane",
